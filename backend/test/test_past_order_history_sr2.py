...
def test_get_certain_past_order_not_found_when_order_does_not_exist():
    response = client.get("/orders/history/user123/nonexistent-order-id")       #Attempt to retrieve an order that does not exist in memory store
...
def test_get_certain_past_orders_show_unauthorized_when_order_belongs_to_another_user():
    ...
    response = client.get(f"/orders/history/user999/{order_id}")        #Attempt to retrieve an order belonging to a different user
...
def test_get_certain_past_order_displays_full_order_details():
    ...
    response = client.get(f"/orders/history/user456/{order_id}")        #Request selected past order using the user and order id
